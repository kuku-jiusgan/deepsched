<template>
  <div>
    ...
    <a-table v-else :dataSource="filtered" :columns="columns" rowKey="id" size="middle"
      :pagination="{ pageSize: 20, showSizeChanger: true, showTotal: (t: number) => '' + t + '' }" :scroll="{ x: 1100 }" />
    ...
    <a-modal :title="editingId ? '编辑仪器' : '添加仪器'" v-model:open="modalOpen" @ok="handleSubmit" width="640"
      ...
      <a-form layout="vertical" style="margin-top: 16px">
        ...
        <a-space v-for="(cap, idx) in form.capabilities" :key="idx" style="display: flex; margin-bottom: 8px" align="baseline">
          ...
          <a-select v-model:value="cap.tag_value" placeholder="标签值" style="width: 160px" :options="(capValueOptions as any)[cap.tag_name] || []" />
...
const capValueOptions = {
  '离子源': [{ label: 'ESI', value: 'ESI' }, { label: 'APCI', value: 'APCI' }],
  '质量分析器': [{ label: 'QqQ', value: 'QqQ' }, { label: 'Q-TOF', value: 'Q-TOF' }, { label: 'Orbitrap', value: 'Orbitrap' }],
