...
import { generateSchedule, calculateInsertCost, confirmInsert, reschedule, dailyRoll, getProjects, getTimeslots } from '../services/api';
import type { InsertCost, TimeSlot } from '../types';
...
const ScheduleManager: React.FC = () => {
  ...
  const handleInsertCalculate = async () => {
    ...
    try {
      ...
      const proj = allProjects.find((p: any) => p.id === insertProjectId);
      const taskIds = proj?.tasks?.filter((t: any) => t.requires_instrument).map((t: any) => t.id) || [];
      const cost = await calculateInsertCost({ project_id: insertProjectId, task_ids: taskIds });
      setInsertCost(cost);
    } catch { message.error('代价计算失败'); }
  };
  ...
  const slotColumns = [
    ...
    { title: '开始', dataIndex: 'plan_start', key: 'start', render: (v: string) => new Date(v).toLocaleString() },
    { title: '结束', dataIndex: 'plan_end', key: 'end', render: (v: string) => new Date(v).toLocaleString() },
    { title: '层级', dataIndex: 'tier', key: 'tier',
      render: (v: string) => <Tag color={v === 'frozen' ? 'blue' : v === 'confirmed' ? 'green' : 'default'}>{v}</Tag> },
    { title: '状态', dataIndex: 'status', key: 'status',
      render: (v: string) => <Tag color={v === 'completed' ? 'green' : v === 'running' ? 'blue' : 'default'}>{v}</Tag> },
  ];
    ...
    <div>
      ...
      <Modal title="插单" open={insertModalOpen} onCancel={() => setInsertModalOpen(false)}
        ...
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          {insertCost && (
            <div>
              <Alert message={'插单将导致总延期 ' + insertCost.total_delay_hours + ' 小时'}
              ...
              {insertCost.displaced_tasks.length > 0 && (
                <Card title="被挤出的任务" size="small">
                  {insertCost.displaced_tasks.map((t, i) => (
                    <Tag key={i} color="orange">{t.project_name} - {t.task_name} (延期{t.delay_hours}h)</Tag>
                  ))}
                </Card>
              )}
              {insertCost.milestone_violations.length > 0 && (
                <Card title="里程碑违约" size="small" style={{ marginTop: 8 }}>
                  {insertCost.milestone_violations.map((v, i) => (
                    <Tag key={i} color="red">{v.project} - {v.milestone} (延期{v.days_late}天)</Tag>
                  ))}
                </Card>
              )}
            </div>
